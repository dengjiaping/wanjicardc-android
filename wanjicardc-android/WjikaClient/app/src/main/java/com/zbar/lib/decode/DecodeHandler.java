...
import com.wjika.client.cardpackage.controller.ScanFragment;
...
final class DecodeHandler extends Handler {
	...
	private void decode(byte[] data, int width, int height) {
		...
		String result = manager.decode(rotatedData, width, height, true, fragment.getX(), fragment.getY(), fragment.getCropWidth(), fragment.getCropHeight());

		if (result != null) {
			if (fragment.isNeedCapture()) {
				// 生成bitmap
				PlanarYUVLuminanceSource source = new PlanarYUVLuminanceSource(rotatedData, width, height,
						fragment.getX(), fragment.getY(), fragment.getCropWidth(), fragment.getCropHeight(), false);
			...
			//解码成功，通过CaptureActivityHandler转发消息
