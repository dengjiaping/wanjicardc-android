...
public class InitializeService extends IntentService {
	...
	private void performInit() {
		try {
			...
			ClientApplication.getAppConfigs(getApplicationContext());
			MobclickAgent.setDebugMode(false);
			MobclickAgent.startWithConfigure(new MobclickAgent.UMAnalyticsConfig(getApplicationContext(), "55a364a667e58ef31d00186e", ClientApplication.getChannel(getApplicationContext())));
			AnalysisUtil.init(getApplicationContext());
			ClientApplication.initBugly(getApplicationContext());
			...
			if (LogUtil.isDebug) {
				Stetho.initialize(Stetho
						...
						.enableWebKitInspector(
								Stetho.defaultInspectorModulesProvider(getApplicationContext())).build());
			}
		} catch (Exception e) {
			//避免第三方应用引起应用崩溃
			e.printStackTrace();
