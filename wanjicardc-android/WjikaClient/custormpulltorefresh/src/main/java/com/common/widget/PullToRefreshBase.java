...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	public final void setPullToRefreshEnabled(boolean enable) {
		if (DEBUG){
			Log.v(LOG_TAG, "setPullToRefreshEnabled : " + enable);
		}
	...
	protected final void onRestoreInstanceState(Parcelable state) {
		if (state instanceof Bundle) {
			...
			if (DEBUG){
				...
				Log.v(LOG_TAG, "onRestoreInstanceState mCurrentMode : " + mCurrentMode);
			}
	...
	private void pullEvent() {
		...
		switch (mCurrentMode) {
			case PULL_FROM_END:
				...
				newScrollValue = Math.min(Math.round(Math.max(initialMotionValue - lastMotionValue, 0) / FRICTION),itemDimension);
				break;
			case PULL_FROM_START:
			default:
