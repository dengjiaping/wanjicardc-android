...
public class RefreshRecyclerView extends RecyclerView {
	...
	public void onScrollStateChanged(int state) {
		...
		if (state == RecyclerView.SCROLL_STATE_IDLE && canLoadMore() && canAddMore  && onRefreshAndLoadMoreListener != null && currentState != REFRESHING && isOnBotom()) {
	...
	public boolean isOnBotom() {
		...
		if (layoutManager instanceof GridLayoutManager) {
			...
			return (mWrapAdapter.getItemCount() - 1) == ((LinearLayoutManager) layoutManager).findFirstVisibleItemPosition();
		}
	}
	...
	class WrapperAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
		...
		public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
			...
			if (viewType == ITEM_TYPE_LOADMORE_FOOTER) {
				return new RecyclerViewHolder(parent.getContext(), mFootViews.get(mFootViews.size() - 1));
			}
